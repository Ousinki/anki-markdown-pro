...
- **公式支持 (MathJax & mhchem)**：内置原生 MathJax 公式渲染支持，支持标准 `$` 和 `$$` 语法，并原生集成 `mhchem` 化学方程式排版（无需任何额外下载），支持 `\ce{...}`
- **全功能 Markdown**：支持加粗、斜体、列表、引用块、表格、直接粘贴图片、警告块、**双等号高亮标记 (`==highlight==`)**、**精美 `<kbd>` 3D 键帽样式**等
- **缩写与悬浮提示 (`[文字]{提示}`)**：支持零打扰的缩写悬浮窗提示语法（如 `[Anki]{记忆卡片工具}`），鼠标悬停或移动端点击时呈现优雅的半透明悬浮信息窗，且具备智能边界溢出躲避定位
- **双模音频管理**：支持粘贴或录制音频，支持自动播放与点击播放（带有小喇叭图标且自适应 iOS/移动端隐藏）两种模式，工具栏支持一键切换插入模式
- **Markdown 挖空卡片 (Cloze Deletion)**：原生支持 Markdown 挖空卡片（`Anki Markdown Cloze` 牌型），在编辑区无缝绑定官方挖空快捷键（`Cmd/Ctrl+Shift+C`）与专属工具栏按钮，支持智能递增挖空与同号挖空
